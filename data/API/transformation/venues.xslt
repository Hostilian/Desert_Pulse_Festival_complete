...
<xsl:template match="/festival">{
  "items": [<xsl:for-each select="venues/venue">{
    "id": "<xsl:call-template name="json-string"><xsl:with-param name="text" select="@id"/></xsl:call-template>", "name": "<xsl:call-template name="json-string"><xsl:with-param name="text" select="name"/></xsl:call-template>", "capacity": <xsl:value-of select="capacity"/>, "description": "<xsl:call-template name="json-string"><xsl:with-param name="text" select="description"/></xsl:call-template>", "area": "<xsl:call-template name="json-string"><xsl:with-param name="text" select="area"/></xsl:call-template>", "geo": "<xsl:call-template name="json-string"><xsl:with-param name="text" select="geo"/></xsl:call-template>"}<xsl:if test="position()!=last()">,</xsl:if></xsl:for-each>],
